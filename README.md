...
and then we can use same workflow to bootstrap a classifier for objects that aren't in YOLO or any pretrained model.

User describes the object in plain language → VLM localizes instances (it would cost about 50 to 100 dollars of api to do so) → crops become a labeled dataset → model trains on them.
